...
def _validate_fact_entry(
    entry: dict
) -> bool:
    """
    Validate that a fact entry contains all required keys.

    Required keys: fact, source_title, source_url

    Args:
        entry: A single fact dict from the LLM response.
...
def _validate_facts(
    ...
    Args:
        facts: Raw list of fact dicts from the LLM.
...
# ==========================================
# SINGLE-PASS FACT EXTRACTION PROMPT
# ==========================================
...
async def summarizer_agent_async(
    ...
    Returns:
        A list containing a single summary dict:
            [{
                ...
                "summary_time": N.N,
                "failed": False  (or True on timeout/failure)
            }]
        This shape is backward-compatible with the workflow.
    """
    logger.info(
        "[SUMMARIZER AGENT] Single-pass extraction started"
    )

    start = time.time()

    # ------------------------------------------------------------------
    # Aggregate sources into a single structured context
    # ------------------------------------------------------------------
    ...
    logger.info(
        f"[SUMMARIZER AGENT] Context size: {context_size} chars"
    )
    ...
    try:
        ...
        logger.info(
            f"[SUMMARIZER AGENT] Facts extracted: {len(facts)}"
        )
...
def summarizer_agent(
    ...
    return asyncio.run(
        summarizer_agent_async(topic, sources)
    )
